use bevy::{prelude::{Query, Transform, With, Commands, Without, Entity, Res, DespawnRecursiveExt}, core::{Timer, Time}, math::{Vec3}};

use crate::components::{Stateful, Player, StateKind, CollisionShape, CollisionMasks, MeleeAttack, Damaged, CurrentHealth, Direction, AI, Ground, Velocity};

pub fn process_player_combat_states (
    mut commands: Commands,
    mut query: Query<(&Stateful, &mut MeleeAttack), With<Player>>
) {
    for (state, melee_attack) in query.iter_mut() {
        if !state.new_state {
            continue;
        }
...
pub fn player_melee_attack (
    mut commands: Commands,
    delta_time: Res<Time>,
    mut query: Query<(&mut MeleeAttack, Entity), (Without<Player>, Without<CollisionShape>)>,
    mut player_transform_query: Query<(&mut Transform, &Direction), With<Player>>
) {
    for (mut melee_attack, entity) in query.iter_mut() {
        ...
        let (mut player_transform, direction) = player_transform_query.single_mut();
        player_transform.translation.x += 10.0 * direction.flip_x;
